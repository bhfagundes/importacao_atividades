...
class EnvioNotaPreNotaController extends AppBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        return view('envio_nota_pre_nota.index')
    ...
    public function store(CreateEnvioNotaServicoRequest $request)
    {
        ...
        $params['of_recebe_pre_nota']=array(
           'astr_recebe'=> array(
                'ocm'=>$input['svc'],
                'arquivo_xml'=>base64_encode(file_get_contents($xml->getRealPath())),
                'arquivo_pdf'=>base64_encode(file_get_contents($pdf->getRealPath()))
            )
        );
        /*$svc = 'SVC'. $input['doc_eletronico'];
        ...
        //dd($token);
        curl_setopt_array($curl, array(
        CURLOPT_URL => "https://hml-api.energisa.io/WSCFSPB_SFC/v1/of_recebe_pre_nota",
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => "",
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 0,
